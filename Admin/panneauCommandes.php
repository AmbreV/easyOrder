...
<form id="tri" method="post" action="" >
  <input name="client" type="text" placeholder="Nom du Client" 
         value="<?php echo $tri[0]; ?>" />
<br />
  du : 
  <select name="duJour">
  <?php for ($i=0; $i<=31; $i++) {
...
if ($tri[4] === '') {
  $req .= '';
} else if ( ($tri[4] == 1) || (intval($tri[4]) === 0) ) {
  if ( $tri[3] == 'on' ) {
    $req .= 'AND Commandes.valide != '.$tri[4].' ';
  } else {
    $req .= 'AND Commandes.valide = '.$tri[4].' ';
  }
} 
...
//while ( $donnees = $resultat->fetch() ) {
  ...
  case 1: echo 'Validée'; break;
    ...
    echo '</td>';
  echo '<form method="post" action="../Commun/consulterCommandes.php" >';
  echo '<td>';
  echo '<input type="hidden" name="consulter" value="'.$d.'" />';
  echo '<input class="consult smallbutton" type="submit" name="submit" value="consulter"';
  /*
